...
app.post('/Signup', async (req, res) => {
  try {
    ...
    if (existingUser) 
    {
      return res.status(400).json({ message: 'Usernamelready exists, Please Sign In to Continue or use diffrent credentiols' });
    }
...
app.delete('/auction/:id', authenticate, async (req, res) => {
  try {
    ...
    if (auctionItem.createdBy.toString() !== req.user.userId) {
